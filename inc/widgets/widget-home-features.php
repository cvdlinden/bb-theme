...
/**
 * Homepage features Widget Class.
 *
 * @see WP_Widget
 */
class BB_Home_Features extends WP_Widget {

	/**
	 * PHP5 constructor.
	 */
	function __construct() {
		$widget_ops = array( 'classname' => 'bb_home_features','description' => esc_html__( 'Widget to set Features in Home Section', 'bb' ) );
		parent::__construct( 'bb_home_features', esc_html__( '[BB] Features Section For FrontPage','bb' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<?php for ( $i = 1; $i < 4; $i++ ) { ?>
			<br>
			<b><?php echo sprintf( __( 'Feature %s', 'bb' ), $i ); ?></b>
			
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' . $i ) ); ?>"><?php esc_html_e( 'Title ','bb' ) ?></label>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'icon' . $i ) ); ?>"><?php esc_html_e( 'Icon( Font Awsome ) ','bb' ) ?></label><?php
				$get_fontawesome_icons = $this->get_fontawesome_icons();
				$icon = ( isset( $instance[ 'icon' . $i ] ) && '' != $instance[ 'icon' . $i ] ) ? esc_html( $instance[ 'icon' . $i ] ): '';?>
				<select class="bb-icon" id="<?php echo esc_attr( $this->get_field_id( 'icon' . $i ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'icon' . $i ) ); ?>">
					<option value=""><?php esc_html_e( 'Select Icon', 'bb' ); ?></option>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'body_content' . $i ) ); ?>"><?php esc_html_e( 'Content ', 'bb' ) ?></label>
	...
	/**
	 * Function for font awesome list
	 */
