							...
							if( $logos['img'] != '' && $logos['link'] != '' ){ ?>
								<li>
									<a href="<?php echo $logos['link'][$i]; ?>">
										<img alt="<?php _e('Logos', 'bb'); ?>" src="<?php echo $logos['img'][$i]; ?>" />
									</a>
								</li><?php
							}
						}?>
	...
	?>
		...
		<ul class="client-sortable clone-wrapper"><?php
			...
			for ( $i = 0; $i < $slider_count; $i++ ): ?>
				<li class="toclone">
				<br>
					<label class="logo_heading"><b><?php _e( 'Logo #', 'bb' ); ?><span class="count"><?php echo absint( $i )+1; ?></span></b></label>
					...
					<button id="<?php echo $this->get_field_id('image_src_button').'-'.$i; ?>" class="button button-primary custom_media_button" data-fieldid="<?php echo $this->get_field_id('image_src').'-'.$i; ?>"><?php _e( 'Upload Logo','bb' ); ?></button>
					...
					<a href="#" class="clone button-primary"><?php _e('Add', 'bb'); ?></a>
					<a href="#" class="delete button"><?php _e('Delete', 'bb'); ?></a>
				</li>
			<?php endfor; ?>
		</ul><?php
