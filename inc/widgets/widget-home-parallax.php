 ...
 * Bij Best Theme
 */
class bb_theme_home_parallax extends WP_Widget
{
    function __construct(){

        $widget_ops = array('classname' => 'bb_theme_home_parallax','description' => esc_html__( "Bij Best FrontPage Parallax Section" ,'bb-theme') );
        parent::__construct('bb_theme_home_parallax', esc_html__('[Bij Best] Parralax Section For FrontPage','bb-theme'), $widget_ops);
    }
    ...
    function widget($args , $instance) {
        ...
        $image_pos = isset($instance['image_pos']) ? $instance['image_pos'] : esc_html__('left' , 'bb-theme');
    ...
    ?>
      ...
      <p>
            <label for="<?php echo $this->get_field_name( 'image_src' ); ?>"><?php _e( 'Image:', 'bb-theme' ); ?></label>
      ...
      <p><label for="<?php echo $this->get_field_id('image_pos'); ?>"><?php esc_html_e('Image Position ','bb-theme') ?></label>
      <select name="<?php echo $this->get_field_name('image_pos'); ?>"
              id="<?php $this->get_field_id('image_pos'); ?>" class="widefat">
              <option value="left" <?php selected( $instance['image_pos'], 'left' ); ?>><?php _e('Left', 'bb-theme'); ?></option>
              <option value="right" <?php selected( $instance['image_pos'], 'right' ); ?>><?php _e('Right', 'bb-theme'); ?></option>
              <option value="top" <?php selected( $instance['image_pos'], 'top' ); ?>><?php _e('Top', 'bb-theme'); ?></option>
              <option value="bottom" <?php selected( $instance['image_pos'], 'bottom' ); ?>><?php _e('Bottom', 'bb-theme'); ?></option>
              <option value="background-full" <?php selected( $instance['image_pos'], 'background-full' ); ?>><?php _e('Background Full', 'bb-theme'); ?></option>
              <option value="background-small" <?php selected( $instance['image_pos'], 'background-small' ); ?>><?php _e('Background Small', 'bb-theme'); ?></option>
      </select>
      </p>

      <p><label for="<?php echo $this->get_field_id('button1'); ?>"><?php esc_html_e('Button 1 Text ','bb-theme') ?></label>
      ...
      <p><label for="<?php echo $this->get_field_id('button1_link'); ?>"><?php esc_html_e('Button 1 Link ','bb-theme') ?></label>
      ...
      <p><label for="<?php echo $this->get_field_id('button2'); ?>"><?php esc_html_e('Button 2 Text ','bb-theme') ?></label>
      ...
      <p><label for="<?php echo $this->get_field_id('button2_link'); ?>"><?php esc_html_e('Button 2 Link ','bb-theme') ?></label>
